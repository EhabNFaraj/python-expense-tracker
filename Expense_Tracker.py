...
def log_expense():
    ...
    category = input("Enter expense category (e.g., Food, Rent, etc.): ").strip()
    ...
    try:
        with open("expenses.csv", mode="r") as file:
            ...
            for row in reader:
                if len(row) < 3:
                    continue
                if row[0].startswith(current_month) and row[1].lower() == category.lower():
                    total += float(row[2])
        ...
        if percentage >= 100:
            print(f"🚨 OVER BUDGET! You've spent ${total:.2f} of your ${limit} {category} budget this month!")
        elif percentage >= 80:
            print(f"⚠️  Warning! You've used {percentage:.0f}% (${total:.2f}/${limit}) of your {category} budget this month.")
        elif percentage >= 50:
            print(f"📊 Heads up: You've used {percentage:.0f}% (${total:.2f}/${limit}) of your {category} budget this month.")
